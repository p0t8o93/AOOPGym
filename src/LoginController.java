...
public class LoginController {
        ...
        public void actionPerformed(ActionEvent e) {
            ...
            if (LoginAuthentication(username, password)) {
                ...
                if (isAdmin(username, password)) {
                    ...
                    UserDashboardController userctrl = new UserDashboardController(userdashboard, id);
                }
            } else {
    ...
    public int getID(String username, String password) {
        ...
        try (Connection conn = DriverManager.getConnection(DB_URL, DB_USER, DB_PASSWORD); PreparedStatement pstmt = conn.prepareStatement(sql)) {
            ...
            if (rs.next()) {
                userID = rs.getInt("UserID"); // Retrieve the UserID
            }
        ...
        return (userID != null) ? userID : -1; // Return -1 if userID is not found
    }
    ...
    public void createTables() {
        ...
        if (doesTableExist("Users")) {
            ...
            try (Connection connection = DriverManager.getConnection(DB_URL, DB_USER, DB_PASSWORD); PreparedStatement preparedStatement = connection.prepareStatement(userTable)) {
                // Execute the query
                ResultSet resultSet = preparedStatement.executeQuery();
            } catch (Exception e) {
                e.printStackTrace();
    ...
    if (userID != null) {
        sql = "INSERT INTO Attendance (Date, UserID, time_in, time_out) VALUES (?, ?, ?, ?)";
        try (Connection conn = DriverManager.getConnection(DB_URL, DB_USER, DB_PASSWORD);
             PreparedStatement pstmt = conn.prepareStatement(sql)) {

            // Set the parameters
            java.sql.Date sqlDate = java.sql.Date.valueOf(getCurrentDate());
            java.sql.Time sqlTimeIn = new java.sql.Time(System.currentTimeMillis()); // Current time as time_in

            pstmt.setDate(1, sqlDate);
            pstmt.setInt(2, userID);
            pstmt.setTime(3, sqlTimeIn);
            pstmt.setNull(4, java.sql.Types.TIME); // Set time_out to NULL
